...
class ForwardMessageSuite extends FunSuite {
  ...
  test("Encode SignedMessage in ForwardMessageJson and in ForwardMessageBase64") {
    val sMsg = SignedMessageExamples.exampleTrustPingFromAliceToBob_obj
    val next = sMsg.payloadAsPlaintextMessage.toOption
      .flatMap(_.to.flatMap(_.headOption.map(_.toDIDSubject)))
      .getOrElse(???) // "did:example:bob"
