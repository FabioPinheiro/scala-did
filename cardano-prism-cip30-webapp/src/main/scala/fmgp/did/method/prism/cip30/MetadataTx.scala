...
/** Build a pay-yourself Cardano transaction whose CIP-10 metadata carries the given PRISM events. Pure functions only
  * -- no `js.Dynamic`, no I/O. The browser caller wires the inputs from CIP-30 (`getUtxos`, `getChangeAddress`) and
  * ships the resulting CBOR to `signTx` / `submitTx`.
  *
  * The body bytes signed by the wallet round-trip unchanged through Scalus's `KeepRaw[TransactionBody]`; re-emitting
  * after merging the witness set yields a tx whose body hash matches what the wallet actually signed.
  */
object MetadataTx:

  /** CIP-30 wallet snapshot, hex-decoded. `utxosCbor` are the raw bytes of each `[input, output]` array as returned by
    * `api.getUtxos()`.
    */
  ...
  /** Refuse if the picked UTxO is below this threshold (covers fee + min-utxo value floor).
    */
  ...
  /** Splice the wallet's witness-set bytes into the unsigned tx so the result is ready for `submitTx`. The body bytes
  ...
  private def merge(a: TransactionWitnessSet, b: TransactionWitnessSet): TransactionWitnessSet =
    a.copy(
      ...
      bootstrapWitnesses = TaggedSortedSet.from(a.bootstrapWitnesses.toSet ++ b.bootstrapWitnesses.toSet),
    )
