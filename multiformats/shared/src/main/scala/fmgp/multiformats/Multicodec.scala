...
case class Multicodec(codec: Codec, dataBytes: Array[Byte]) {
  def warp: MulticodecBytes = MulticodecBytes.unsafe(codec.varint.bytes ++ dataBytes)
  def bytes = warp.bytes
}
object Multicodec {
  def unsafeFromBytes(bytes: Array[Byte]): Multicodec = {
    val (code, offset) = Varint.unsafe(bytes).decodeInt
    Codec.values.find(_.code == code) match
      case None        => throw new IllegalArgumentException(s"Unknown Multicodec '$code'")
      case Some(codec) => Multicodec(codec, bytes.drop(offset))
  }
  def fromBytes(bytes: Array[Byte]): Either[String, Multicodec] = {
    val (code, offset) = Varint.unsafe(bytes).decodeInt
    Codec.values.find(_.code == code) match
      case None        => Left(s"Unknown Multicodec '$code'")
      case Some(codec) => Right(Multicodec(codec, bytes.drop(offset)))
  }
}

opaque type MulticodecBytes = Array[Byte]
object MulticodecBytes {
  def unsafe(bytes: Array[Byte]): MulticodecBytes = bytes
  def apply(bytes: Array[Byte]): Either[String, MulticodecBytes] = Multicodec.fromBytes(bytes).map(_.warp)
  extension (data: MulticodecBytes)
    def bytes: Array[Byte] = data
    def unwrap: Multicodec = {
      val (code, offset) = Varint.unsafe(data).decodeInt
      Codec.values.find(_.code == code) match
        case None        => ??? // FIXME
        case Some(codec) => Multicodec(codec, bytes.drop(offset))
    }
...
enum CodecTag {
  case SERIALIZATION, MULTIFORMAT, MULTIHASH, MULTIADDR, IPLD, NAMESPACE, CID, HASH, KEY, LIBP2P, TRANSPORT, MULTISIG,
    VARSIG, VALD, NONCE, MULTIKEY, ENCRYPTION, FILECOIN, HOLOCHAIN, SOFTHASH, ZEROCERT
}
