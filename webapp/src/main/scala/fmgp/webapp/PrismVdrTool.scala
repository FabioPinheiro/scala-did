...
object PrismVdrTool {
  ...
  def opSignal = dataVar.signal
    ...
    .map(
      _.map((rawData, index) =>
        val htmlElement = rawData.fromJson[MySignedPrismEvent[OP]] match
          case Left(error) =>
            Try {
              val protobufOP = PrismEvent.parseFrom(hex2bytes(rawData)) // FIXME catch exceptions
              val op = OP.fromPrismEvent(protobufOP)
              div(
              ...
              case Failure(exception) =>
                div(
                  ...
                  ul(
                    li(s"Not a MySignedPrismEvent due to: $error"),
                    li(s"Not a PrismEvent due to: ${exception.getMessage}"),
                  )
                )
          case Right(op: MySignedPrismEvent[OP]) =>
            div(
  ...
  def apply(): HtmlElement = div(
    h1("PRISM VDR Tool"),
    div(p("Input a list (separated by line) of MySignedPrismEvent[OP] or proto.PrismEvent")), // FIXME REMOVE
    div(
      textArea(
        rows := 10,
        cols := 80,
