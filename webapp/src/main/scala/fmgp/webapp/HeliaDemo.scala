...
import typings.helia

@js.annotation.JSExportTopLevel("HeliaDemo")
object HeliaDemo {

  @js.annotation.JSExport
  def test() = {
    println("Start Helia")
    val p = helia.mod.createHelia()

    // p.`then`(e =>
    //   println("Helia is running")
    //   println("PeerId:" + e.libp2p.peerId.toString())
    // )
    // p
  }
